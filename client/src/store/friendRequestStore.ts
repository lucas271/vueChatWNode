...
export const useFriendRequestStore =  defineStore('userRequest', () => {
    ...
    async function sendRequest(receiverId: string, senderId: string, page:number){
        ...
        const sendRequest = await axios.post('https://vuechatwnodeapi-jt77.onrender.com/sendFriendRequest', {senderId, receiverId}).then(res => res.data.response).catch(async res => {
    ...
    async function getRequests(receiverId: string){
        ...
        const requests = await axios.get('https://vuechatwnodeapi-jt77.onrender.com/getFriendRequests'+`?receiverId=${receiverId}`).then(res => res.data.response).catch(async res => {
            loading.value = false
            return res.response?.data?.errors ? errors.value.push(...res.response.data.errors) : errors.value.push('servidor offline')
        })
    ...
    async function friendRequestResponse(receiverId: string, senderId: string, isAccept: boolean, friendRequestId: any){
        reset()
        const response = await axios.put('https://vuechatwnodeapi-jt77.onrender.com/friendRequestResponse', {receiverId, senderId, isAccept, friendRequestId}).then(res => res.data.response).catch(async res => {
            loading.value = false
            return res.response?.data?.errors ? errors.value.push(...res.response.data.errors) : errors.value.push('servidor offline')
        })
